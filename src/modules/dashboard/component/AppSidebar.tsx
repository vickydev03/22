  ...
  return (
    <Sidebar>
      ...
      <SidebarContent className="px-4 mt-6 space-y-2">
        ...
        <Collapsible open={open === "video's"}>
          <CollapsibleTrigger
            onClick={() => toggle("video's")}
            className="flex items-center justify-between w-full p-2 rounded-md hover:bg-muted transition"
          >
            <div className="flex items-center gap-3">
              <FileVideo size={18} />
          ...
          <CollapsibleContent className="relative pl-10 pt-3 pb-2 space-y-3">
            ...
            {[
              { name: "all_videos", href: "/dashboard/video/all_videos" },
              { name: "upload_videos", href: "/dashboard/video/upload_videos"},
            ].map((item) => {
              const isActive = pathname === item.href;
